...
import 'package:recuperacion/models/retardo_model.dart';
...
class RetardoPage extends StatefulWidget {
  static const String routeName = 'retardo';
  ...
  @override
  State<RetardoPage> createState() => _RetardoPageState();
}

class _RetardoPageState extends State<RetardoPage> {
  String? _tipoapp;
  List<Retardo> results = [], originals = [];
    ...
    try {
      var res = await http.post(
        Uri(
          // scheme: 'http',
          // host: '192.168.1.77',
          // path: '/registroAsistencia/public/asistencia/getAllRetardos2/',
          scheme: 'https',
  ...
  Widget build(BuildContext context) {
          ...
          return WillPopScope(
            ...
            child: Scaffold(
              ...
              appBar: AppBar(
                  ...
                  title: const Text('Retardos', style: TextStyle(fontSize: 20)),
                  centerTitle: true,
    ...
    //                 // automaticallyImplyLeading: false,
    //                 title:
    //                     const Text('Retardos', style: TextStyle(fontSize: 20)),
    //                 centerTitle: true,
    //                 backgroundColor: const Color.fromRGBO(55, 171, 204, 1),
    //                 actions: <Widget>[
    //                   PopupMenuButton(
    ...
    //                     AsyncSnapshot<AppDataRetardo> snapshot) {
    //                   if (!snapshot.hasData) {
    //                     return const Center(
    ...
    //                   } else {
    //                     if (results.isEmpty) {
    //                       results = snapshot.data!.retardo;
    //                       originals = snapshot.data!.retardo;
    //                     }
    //                   }
    ...
    //                         barrierColor: Colors.black.withOpacity(0.6),
    //                         opaque: false,
    //                         pageBuilder: (_, __, ___) => const RetardoPage(),
    //                         transitionDuration:
    //                             const Duration(milliseconds: 800),
    //                         transitionsBuilder: (_, animation, __, child) {
    //                           return BackdropFilter(
    ...
    //                                   itemBuilder: (context, index) {
    //                                     final Retardo retard = results[index];
    //                                     final colors =
  ...
  Future<bool> _onWillPop2(id, nombre, noEmpleado) async {
    return (await showDialog(
          barrierDismissible: false,
          context: context,
          builder: (context) => AlertDialog(
            ...
            actions: <Widget>[
              ...
              ElevatedButton(
                onPressed: () async {
                  showProgress(context, id, noEmpleado);
                },
                child: const Text('Si'),
              ),
            ],
  ...
  showProgress(BuildContext context, String id, String noEmpleado) async {
    var result = await showDialog(
      context: context,
      builder: (context) => FutureProgressDialog(_retardo(id, noEmpleado)),
    );
    return result;
  }

  Future<void> _retardo(id, noEmpleado) async {
    try {
      String pat = '';
      pat = '/registroAsistencia/public/asistencia/justificarRetardo/' +
          id +
          '/' +
          noEmpleado;
      ...
      if (response.statusCode == 200) {
        ...
        HapticFeedback.heavyImpact();
        AnimatedSnackBar.material('Ocurrio algo extraño, vuelve a intentarlo',
                type: AnimatedSnackBarType.warning,
                mobileSnackBarPosition: MobileSnackBarPosition.top,
                duration: const Duration(milliseconds: 400))
            .show(context)
